...
def testExe(exePath, exeWD):
    ...
    outputs = []
    ret = True
    with open(exeWD + "/output.txt") as f: outputs = [line.rstrip() for line in f]
    try:
        proc = subprocess.run([exePath], capture_output = True, cwd = exeWD)
        J = json.loads(proc.stdout.decode("utf-8"))
        if outputs[0] == str(J["output1"]):
            print("Part 1 OK")
        else:
            print(f"Part 1 FAILED: {outputs[0]} != {J['output1']}")
            ret = False
        if outputs[1] == str(J["output2"]):
            print("Part 2 OK")
        else:
            print(f"Part 2 FAILED: {outputs[1]} != {J['output2']}")
            ret = False
